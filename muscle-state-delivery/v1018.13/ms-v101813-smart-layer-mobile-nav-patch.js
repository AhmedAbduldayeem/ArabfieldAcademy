...
if(!h.includes('ms101813-weight-cta')){
  ...
  const cta=`<a class="ms101813-weight-cta" href="./weights.html#weightsWorkspace"><span><strong>تسجيل الأوزان</strong><small>افتح سجل أوزانك وسجّل أرقامك الحالية من داخل التمرين.</small></span><b>↗</b></a>`;
  h=h.replace(anchor,cta+anchor);
}
h=h.replace(/<link rel="stylesheet" href="assets\/css\/ms-v101812-mobile-nav\.css\?v=\d+">\s*/g,'');
if(!h.includes('assets/css/ms-v101813-mobile-nav.css'))h=h.replace('</head>','<link rel="stylesheet" href="assets/css/ms-v101813-mobile-nav.css?v=101813">\n</head>');
h=h.replace(/assets\/js\/ms-food-photo-v1018-1\.js\?v=\d+/,'assets/js/ms-food-photo-v1018-1.js?v=101813');
h=h.replace(/assets\/js\/ms-food-photo-result-v10182\.js\?v=\d+/,'assets/js/ms-food-photo-result-v10182.js?v=101813');
h=h.replace(/assets\/js\/pwa\.js\?v=\d+/,'assets/js/pwa.js?v=101813');
write('app.html',h);

let f=read('assets/js/ms-food-photo-v1018-1.js');
f=f.replace(/const max=\d+/, 'const max=1024');
f=f.replace(/cv\.toBlob\(r,'image\/webp',\.[0-9]+\)/,"cv.toBlob(r,'image/webp',.72)");
f=f.replace(/cv\.toBlob\(r,'image\/jpeg',\.[0-9]+\)/,"cv.toBlob(r,'image/jpeg',.72)");
f=f.replace(/blob\?\.size>\d+/, 'blob?.size>1200000');
const jpegMatches=[...f.matchAll(/cv\.toBlob\(r,'image\/jpeg',\.[0-9]+\)/g)];
if(jpegMatches.length>1){const last=jpegMatches[jpegMatches.length-1][0];f=f.slice(0,jpegMatches[jpegMatches.length-1].index)+last.replace(/\.[0-9]+\)/,'.52)')+f.slice(jpegMatches[jpegMatches.length-1].index+last.length)}
...
p=p.replace(/const VERSION='\d+';/,"const VERSION='101813';");
write('assets/js/pwa.js',p);
console.log('V101813_IDENTITY_SAFE_SMART_NAV_PATCH_OK');
