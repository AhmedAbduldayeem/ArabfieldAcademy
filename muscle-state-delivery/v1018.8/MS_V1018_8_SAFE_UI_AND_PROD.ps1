...
$base='https://raw.githubusercontent.com/AhmedAbduldayeem/ArabfieldAcademy/'+$branch+'/muscle-state-delivery/v1018.8'
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$rb=Join-Path $root ('_rollback\v1018.8-safe-ui-'+$stamp)
...
Write-Host '=== MUSCLE STATE v1018.8 SAFE UI ==='
...
$patch=Join-Path $env:TEMP 'MS_V10188_SAFE_UI_PATCH.js'
$ok=$false
try{
 Invoke-WebRequest -UseBasicParsing -Uri ($base+'/ms-v10188-safe-ui-patch.js') -OutFile $patch -TimeoutSec 60
 $ph=Get-GitBlobSha1 $patch;Write-Host ('PATCH_GIT_BLOB_SHA1='+$ph);if($ph -ne 'e5f72e2a5c140f9c20fd5623d8a25e9188ba77f8'){throw ('PATCH_BLOB_MISMATCH '+$ph)}
 ...
 if(!$f.Contains("provider_quota_or_billing:'تعذر تشغيل التحليل الآن. جرّب مرة أخرى بعد قليل.'")){throw 'GENERIC_BILLING_ERROR_COPY_MISSING'}
 if(!$f.Contains("provider_model_unavailable:'التحليل غير متاح مؤقتًا. جرّب بعد قليل.'")){throw 'GENERIC_MODEL_ERROR_COPY_MISSING'}
 if(!$r.Contains("provider_quota_or_billing:'تعذر تشغيل التحليل الآن. جرّب مرة أخرى بعد قليل.'")){throw 'RESULT_GENERIC_BILLING_COPY_MISSING'}
 if(!$h.Contains('ms-food-photo-v1018-1.js?v=10188')){throw 'FOOD_CACHE_10188_MISSING'}
 if(!$h.Contains('ms-food-photo-result-v10182.js?v=10188')){throw 'RESULT_CACHE_10188_MISSING'}
 if(!$h.Contains('assets/js/pwa.js?v=10188')){throw 'PWA_QUERY_10188_MISSING'}
 if(!$p.Contains("const VERSION='10188';")){throw 'PWA_VERSION_10188_MISSING'}
 Write-Host 'V1018_8_SOURCE_ASSERTIONS_OK'
 Write-Host 'user_ui=PRODUCT_COPY_ONLY'
 Write-Host 'internal_identifiers=PRESERVED'
 Write-Host 'quoted_string_scan=SAFE'
 Write-Host 'health_success_message=HIDDEN'
 Write-Host 'pwa_cache=10188'
 ...
 Write-Host ('V1018.8 ERROR: '+$_.Exception.Message) -ForegroundColor Red
...
if($ok){Write-Host 'V1018.8 LOCAL QA PASS' -ForegroundColor Green;Write-Host '=== DEPLOYING OFFICIAL PRODUCTION ===';vercel.cmd --prod --yes --scope dayeem-studio-demos;if($LASTEXITCODE -ne 0){Write-Host 'V1018.8 LOCAL PASS BUT PRODUCTION DEPLOY FAILED' -ForegroundColor Yellow;exit $LASTEXITCODE};Write-Host 'V1018.8 PRODUCTION DEPLOY COMPLETED' -ForegroundColor Green}