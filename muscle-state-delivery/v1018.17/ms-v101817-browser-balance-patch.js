...
// A) Browser/desktop cleanup. Use a final override instead of editing an existing media block.
let css=read('assets/css/ms-v101815-clean-mobile.css');
const marker='/* V101817_BROWSER_DOCK_FINAL_OVERRIDE */';
...
// B) Replace training sessions and synchronize public program metadata.
...
for(const file of ['app.html','weights.html','programs.html']){
  ...
  h=h.replace(/assets\/js\/data\.js\?v=\d+/g,'assets/js/data.js?v=101817');
  h=h.replace(/ms-v101815-clean-mobile\.css\?v=\d+/g,'ms-v101815-clean-mobile.css?v=101817');
  h=h.replace(/ms-v101815-clean-mobile\.js\?v=\d+/g,'ms-v101815-clean-mobile.js?v=101817');
  h=h.replace(/assets\/js\/pwa\.js\?v=\d+/g,'assets/js/pwa.js?v=101817');
  h=h.replace(/manifest\.webmanifest\?v=\d+/g,'manifest.webmanifest?v=101817');
  fs.writeFileSync(full,h,'utf8');
}
let pwa=read('assets/js/pwa.js');
pwa=pwa.replace(/const VERSION='\d+';/,"const VERSION='101817';");
write('assets/js/pwa.js',pwa);
let sw=read('sw.js');
sw=sw.replace(/const VERSION='muscle-state-pwa-v\d+';/,"const VERSION='muscle-state-pwa-v101817';");
write('sw.js',sw);

console.log('V101817_BROWSER_BALANCE_PATCH_OK');
