...
$base='https://raw.githubusercontent.com/AhmedAbduldayeem/ArabfieldAcademy/'+$branch+'/muscle-state-delivery/v1018.12'
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$rb=Join-Path $root ('_rollback\v1018.12-smart-quota-nav-'+$stamp)
...
$cssRel='assets\css\ms-v101812-mobile-nav.css'
...
Write-Host '=== MUSCLE STATE v1018.12 SMART QUOTA + MOBILE NAV ==='
...
$patch=Join-Path $env:TEMP 'MS_V101812_SMART_QUOTA_NAV_PATCH.js'
$ok=$false
try{
 Download-Verified 'ms-v101812-smart-layer-mobile-nav-patch.js' $patch '77dad39043582a281cebe0bf3f8275b6fffdc51e'
 Download-Verified 'ms-v101812-mobile-nav.css' $cssPath 'eb4285c8fa610fffc04d00c721e773a2a57be31f'
 ...
 if(!$h.Contains('ms101812-weight-cta') -or !$h.Contains('./weights.html#weightsWorkspace')){throw 'TRAINING_WEIGHTS_CTA_MISSING'}
 if(!$h.Contains('assets/css/ms-v101812-mobile-nav.css?v=101812')){throw 'MOBILE_CSS_REF_MISSING'}
 if(!$h.Contains('ms-food-photo-v1018-1.js?v=101812')){throw 'FOOD_CACHE_101812_MISSING'}
 if(!$h.Contains('ms-food-photo-result-v10182.js?v=101812')){throw 'RESULT_CACHE_101812_MISSING'}
 if(!$h.Contains('assets/js/pwa.js?v=101812')){throw 'PWA_QUERY_101812_MISSING'}
 if(!$p.Contains("const VERSION='101812';")){throw 'PWA_VERSION_101812_MISSING'}
 ...
 foreach($v in @('#fff7e8','#f6ead7','#6da77a','#c9ff3d','#214b31')){if(!$c.Contains($v)){throw ('IDENTITY_COLOR_MISSING '+$v)}}
 Write-Host 'V1018_12_SOURCE_ASSERTIONS_OK'
 ...
 Write-Host ('V1018.12 ERROR: '+$_.Exception.Message) -ForegroundColor Red
...
if($ok){
 Write-Host 'V1018.12 LOCAL QA PASS' -ForegroundColor Green
 Write-Host '=== DEPLOYING OFFICIAL PRODUCTION ==='
 vercel.cmd --prod --yes --scope dayeem-studio-demos
 if($LASTEXITCODE -ne 0){Write-Host 'V1018.12 LOCAL PASS BUT PRODUCTION DEPLOY FAILED' -ForegroundColor Yellow;exit $LASTEXITCODE}
 $live=(Invoke-WebRequest -UseBasicParsing -Uri ('https://muscle-state-00.vercel.app/app.html?v=101812&t='+(Get-Date).Ticks) -TimeoutSec 45).Content
 ...
 if(!$live.Contains('ms101812-weight-cta') -or !$live.Contains('ms-food-photo-v1018-1.js?v=101812') -or !$live.Contains('assets/js/pwa.js?v=101812')){throw 'PRODUCTION_101812_ASSERTION_FAILED'}
 Write-Host 'PRODUCTION_ASSERTIONS=PASS'
 Write-Host 'V1018.12 PRODUCTION DEPLOY COMPLETED' -ForegroundColor Green
}
