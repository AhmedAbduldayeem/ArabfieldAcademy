...
$base='https://raw.githubusercontent.com/AhmedAbduldayeem/ArabfieldAcademy/'+$branch+'/muscle-state-delivery/v1018.10'
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$rb=Join-Path $root ('_rollback\v1018.10-simple-ui-'+$stamp)
...
Write-Host '=== MUSCLE STATE v1018.10 SIMPLE UI ==='
...
$patch=Join-Path $env:TEMP 'MS_V101810_SIMPLE_UI_PATCH.js'
$ok=$false
try{
 Invoke-WebRequest -UseBasicParsing -Uri ($base+'/ms-v101810-simple-ui-patch.js') -OutFile $patch -TimeoutSec 60
 $ph=Get-GitBlobSha1 $patch;Write-Host ('PATCH_GIT_BLOB_SHA1='+$ph);if($ph -ne 'c8fe38e8a8bad019dea236f1f5ebb48bccd53209'){throw ('PATCH_BLOB_MISMATCH '+$ph)}
 ...
 if(!$h.Contains('ms-food-photo-v1018-1.js?v=101810')){throw 'FOOD_CACHE_101810_MISSING'}
 if(!$h.Contains('ms-food-photo-result-v10182.js?v=101810')){throw 'RESULT_CACHE_101810_MISSING'}
 if(!$h.Contains('assets/js/pwa.js?v=101810')){throw 'PWA_QUERY_101810_MISSING'}
 if(!$p.Contains("const VERSION='101810';")){throw 'PWA_VERSION_101810_MISSING'}
 Write-Host 'V1018_10_SOURCE_ASSERTIONS_OK'
 ...
 Write-Host ('V1018.10 ERROR: '+$_.Exception.Message) -ForegroundColor Red
...
if($ok){Write-Host 'V1018.10 LOCAL QA PASS' -ForegroundColor Green;Write-Host '=== DEPLOYING OFFICIAL PRODUCTION ===';vercel.cmd --prod --yes --scope dayeem-studio-demos;if($LASTEXITCODE -ne 0){Write-Host 'V1018.10 LOCAL PASS BUT PRODUCTION DEPLOY FAILED' -ForegroundColor Yellow;exit $LASTEXITCODE};Write-Host 'V1018.10 PRODUCTION DEPLOY COMPLETED' -ForegroundColor Green}
