...
$base18='https://raw.githubusercontent.com/AhmedAbduldayeem/ArabfieldAcademy/'+$branch+'/muscle-state-delivery/v1018.18'
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$rb=Join-Path $root ('_rollback\v1018.18-balance-audit-'+$stamp)
...
Write-Host '=== MUSCLE STATE v1018.18 BALANCE AUDIT + BROWSER FIX ==='
...
$plans=Join-Path $env:TEMP 'MS_V101818_BALANCED_PLANS.json'
$patch=Join-Path $env:TEMP 'MS_V101818_BROWSER_BALANCE_PATCH.js'
$qa=Join-Path $env:TEMP 'MS_V101818_TRAINING_QA.js'
$ok=$false
try{
 ...
 if(!$sw.Contains("const VERSION='muscle-state-pwa-v101818';")){throw 'SW_VERSION_101818_MISSING'}
 ...
 Write-Host 'upper_rule=THREE_CHEST_THREE_BACK_FINISHER'
 Write-Host 'upper_cardio_rule=THREE_CHEST_THREE_BACK_CARDIO'
 Write-Host 'full_body_rule=THREE_UPPER_THREE_LOWER_FINISHER'
...
if($ok){
 Write-Host 'V1018.18 LOCAL QA PASS' -ForegroundColor Green
 Write-Host '=== DEPLOYING OFFICIAL PRODUCTION ==='
 vercel.cmd --prod --yes --scope dayeem-studio-demos
 if($LASTEXITCODE -ne 0){Write-Host 'V1018.18 LOCAL PASS BUT PRODUCTION DEPLOY FAILED' -ForegroundColor Yellow;exit $LASTEXITCODE}
 $q=[string](Get-Date).Ticks
 $liveApp=(curl.exe -L -sS ('https://muscle-state-00.vercel.app/app.html?v=101818&t='+$q)) -join "`n"
 $liveCss=(curl.exe -L -sS ('https://muscle-state-00.vercel.app/assets/css/ms-v101815-clean-mobile.css?v=101818&t='+$q)) -join "`n"
 $liveRuntime=(curl.exe -L -sS ('https://muscle-state-00.vercel.app/assets/js/ms-v101815-clean-mobile.js?v=101818&t='+$q)) -join "`n"
 $liveData=(curl.exe -L -sS ('https://muscle-state-00.vercel.app/assets/js/data.js?v=101818&t='+$q)) -join "`n"
 $liveSw=(curl.exe -L -sS ('https://muscle-state-00.vercel.app/sw.js?v=101818&t='+$q)) -join "`n"
 ...
 Write-Host 'V1018.18 PRODUCTION DEPLOY COMPLETED' -ForegroundColor Green
}