...
$base='https://raw.githubusercontent.com/AhmedAbduldayeem/ArabfieldAcademy/'+$branch+'/muscle-state-delivery/v1018.7'
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$rb=Join-Path $root ('_rollback\v1018.7-safe-ui-'+$stamp)
...
Write-Host '=== MUSCLE STATE v1018.7 SAFE UI ==='
...
$patch=Join-Path $env:TEMP 'MS_V10187_SAFE_UI_PATCH.js'
$ok=$false
try{
 Invoke-WebRequest -UseBasicParsing -Uri ($base+'/ms-v10187-safe-ui-patch.js') -OutFile $patch -TimeoutSec 60
 $ph=Get-GitBlobSha1 $patch;Write-Host ('PATCH_GIT_BLOB_SHA1='+$ph);if($ph -ne '7c912249ca0053ca04ed4c37b6411b034eb80212'){throw ('PATCH_BLOB_MISMATCH '+$ph)}
 ...
 if(!$h.Contains('ms-food-photo-v1018-1.js?v=10187')){throw 'FOOD_CACHE_10187_MISSING'}
 if(!$h.Contains('ms-food-photo-result-v10182.js?v=10187')){throw 'RESULT_CACHE_10187_MISSING'}
 if(!$h.Contains('assets/js/pwa.js?v=10187')){throw 'PWA_QUERY_10187_MISSING'}
 if(!$p.Contains("const VERSION='10187';")){throw 'PWA_VERSION_10187_MISSING'}
 Write-Host 'V1018_7_SOURCE_ASSERTIONS_OK'
 Write-Host 'user_ui=PRODUCT_COPY_ONLY'
 Write-Host 'internal_error_keys=PRESERVED'
 Write-Host 'health_success_message=HIDDEN'
 Write-Host 'pwa_cache=10187'
 ...
 Write-Host ('V1018.7 ERROR: '+$_.Exception.Message) -ForegroundColor Red
...
if($ok){Write-Host 'V1018.7 LOCAL QA PASS' -ForegroundColor Green;Write-Host '=== DEPLOYING OFFICIAL PRODUCTION ===';vercel.cmd --prod --yes --scope dayeem-studio-demos;if($LASTEXITCODE -ne 0){Write-Host 'V1018.7 LOCAL PASS BUT PRODUCTION DEPLOY FAILED' -ForegroundColor Yellow;exit $LASTEXITCODE};Write-Host 'V1018.7 PRODUCTION DEPLOY COMPLETED' -ForegroundColor Green}