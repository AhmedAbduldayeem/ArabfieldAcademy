...
function replaceErrorValue(s,key,value){
  const re=new RegExp('('+key+":\\s*)'[^']*'",'g');
  return s.replace(re,(_,prefix)=>prefix+"'"+value+"'");
}
function assertNoExternalCopy(s,rel){
  const pats=[/'[^']*(?:Gemini|OpenAI|Billing)[^']*'/gi,/"[^"]*(?:Gemini|OpenAI|Billing)[^"]*"/gi,/`[^`]*(?:Gemini|OpenAI|Billing)[^`]*`/gi];
  for(const re of pats){const m=s.match(re);if(m)throw Error('EXTERNAL_USER_COPY_REMAINS '+rel+' '+m[0].slice(0,120));}
}
function cleanFood(rel){
  let s=read(rel);
  ...
  assertNoExternalCopy(s,rel);
  write(rel,s);
}
cleanFood('assets/js/ms-food-photo-v1018-1.js');
cleanFood('assets/js/ms-food-photo-result-v10182.js');
let h=read('app.html');
h=h.replace(/assets\/js\/ms-food-photo-v1018-1\.js\?v=\d+/,'assets/js/ms-food-photo-v1018-1.js?v=10187');
h=h.replace(/assets\/js\/ms-food-photo-result-v10182\.js\?v=\d+/,'assets/js/ms-food-photo-result-v10182.js?v=10187');
h=h.replace(/assets\/js\/pwa\.js\?v=\d+/,'assets/js/pwa.js?v=10187');
write('app.html',h);
let p=read('assets/js/pwa.js');p=p.replace(/const VERSION='\d+';/,"const VERSION='10187';");write('assets/js/pwa.js',p);
console.log('V10187_SAFE_UI_PATCH_OK');