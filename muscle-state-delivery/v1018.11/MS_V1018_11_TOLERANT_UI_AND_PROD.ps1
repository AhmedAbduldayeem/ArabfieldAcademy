...
$base='https://raw.githubusercontent.com/AhmedAbduldayeem/ArabfieldAcademy/'+$branch+'/muscle-state-delivery/v1018.11'
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$rb=Join-Path $root ('_rollback\v1018.11-tolerant-ui-'+$stamp)
...
Write-Host '=== MUSCLE STATE v1018.11 TOLERANT UI ==='
...
$patch=Join-Path $env:TEMP 'MS_V101811_TOLERANT_UI_PATCH.js'
$ok=$false
try{
 Invoke-WebRequest -UseBasicParsing -Uri ($base+'/ms-v101811-tolerant-ui-patch.js') -OutFile $patch -TimeoutSec 60
 $ph=Get-GitBlobSha1 $patch;Write-Host ('PATCH_GIT_BLOB_SHA1='+$ph);if($ph -ne 'd26cda1302b35fecc5575c55064bce3887f64dc4'){throw ('PATCH_BLOB_MISMATCH '+$ph)}
 ...
 if($r -cmatch 'Gemini|OpenAI|Billing'){throw 'RESULT_EXTERNAL_COPY_REMAINS'}
 if(!$h.Contains('ms-food-photo-v1018-1.js?v=101811')){throw 'FOOD_CACHE_101811_MISSING'}
 if(!$h.Contains('ms-food-photo-result-v10182.js?v=101811')){throw 'RESULT_CACHE_101811_MISSING'}
 if(!$h.Contains('assets/js/pwa.js?v=101811')){throw 'PWA_QUERY_101811_MISSING'}
 if(!$p.Contains("const VERSION='101811';")){throw 'PWA_VERSION_101811_MISSING'}
 Write-Host 'V1018_11_SOURCE_ASSERTIONS_OK'
 Write-Host 'powershell_encoding=ASCII_ONLY'
 Write-Host 'ui_scan=CASE_SENSITIVE_SIMPLE'
 Write-Host 'optional_error_keys=TOLERATED'
 Write-Host 'health_success_message=HIDDEN_WHEN_MATCHED'
 Write-Host 'pwa_cache=101811'
 ...
 Write-Host ('V1018.11 ERROR: '+$_.Exception.Message) -ForegroundColor Red
...
if($ok){Write-Host 'V1018.11 LOCAL QA PASS' -ForegroundColor Green;Write-Host '=== DEPLOYING OFFICIAL PRODUCTION ===';vercel.cmd --prod --yes --scope dayeem-studio-demos;if($LASTEXITCODE -ne 0){Write-Host 'V1018.11 LOCAL PASS BUT PRODUCTION DEPLOY FAILED' -ForegroundColor Yellow;exit $LASTEXITCODE};Write-Host 'V1018.11 PRODUCTION DEPLOY COMPLETED' -ForegroundColor Green}
