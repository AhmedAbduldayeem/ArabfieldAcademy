...
$rb=Join-Path $root ('_rollback\v1016.4-real-fix-'+$stamp)
...
Write-Host '=== MUSCLE STATE v1016.4 REAL FIX ==='
...
try{
  ...
  Write-Host 'V1016_4_BEHAVIOR_ASSERTIONS_OK'
  Write-Host 'runtime_reads_live_data=true'
  Write-Host 'runtime_load_order=defer_after_app_runtime'
  ...
  Write-Host ('V1016.4 ERROR: '+$_.Exception.Message) -ForegroundColor Red
  foreach($f in $tracked){
    $src=Join-Path $rb $f
...
if($ok){
  Write-Host 'V1016.4 LOCAL QA PASS' -ForegroundColor Green
  Write-Host '=== DEPLOYING OFFICIAL PRODUCTION ==='
  vercel.cmd --prod --yes --scope dayeem-studio-demos
  if($LASTEXITCODE -ne 0){
    Write-Host 'V1016.4 LOCAL PASS BUT PRODUCTION DEPLOY FAILED' -ForegroundColor Yellow
    exit $LASTEXITCODE
  }
  Write-Host 'V1016.4 PRODUCTION DEPLOY COMPLETED' -ForegroundColor Green
}
