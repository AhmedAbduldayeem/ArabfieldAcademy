...
$base='https://raw.githubusercontent.com/AhmedAbduldayeem/ArabfieldAcademy/'+$branch+'/muscle-state-delivery/v1018.16'
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$rb=Join-Path $root ('_rollback\v1018.16-browser-balance-'+$stamp)
...
Write-Host '=== MUSCLE STATE v1018.16 BROWSER FIX + BALANCED TRAINING ==='
...
$plans=Join-Path $env:TEMP 'MS_V101816_BALANCED_PLANS.json'
$patch=Join-Path $env:TEMP 'MS_V101816_BROWSER_BALANCE_PATCH.js'
$qa=Join-Path $env:TEMP 'MS_V101816_TRAINING_QA.js'
$ok=$false
try{
 ...
 Download-Verified 'ms-v101816-balanced-plans.json' $plans 'a5a70f30f03e69b01f86addca89eea0058d39cbf'
 Download-Verified 'ms-v101816-browser-balance-patch.js' $patch '6e11d0f0d32feb160dfbdcfc49e5b6d7cb8431f7'
 Download-Verified 'ms-v101816-training-qa.js' $qa '622a26a981af895f325b558d4d8f0cc130b0e778'
 ...
 if(!$css.Contains('.mobile-nav.ms938-nav{display:none!important}') -or !$css.Contains('.ms101815-bottom-nav{display:none!important}') -or !$css.Contains('.ms101815-bottom-nav{display:grid!important}')){throw 'BROWSER_DOCK_CSS_ASSERTION_FAILED'}
 ...
 foreach($h in @($app,$weights,$programs)){if(!$h.Contains('assets/js/data.js?v=101816')){throw 'DATA_CACHE_REF_101816_MISSING'}}
 foreach($h in @($app,$weights)){if(!$h.Contains('ms-v101815-clean-mobile.css?v=101816') -or !$h.Contains('ms-v101815-clean-mobile.js?v=101816')){throw 'CLEAN_MOBILE_CACHE_REF_101816_MISSING'}}
 if(!$pwa.Contains("const VERSION='101816';")){throw 'PWA_VERSION_101816_MISSING'}
 if(!$sw.Contains("const VERSION='muscle-state-pwa-v101816';")){throw 'SW_VERSION_101816_MISSING'}
 Write-Host 'V1018_16_SOURCE_ASSERTIONS_OK'
 Write-Host 'desktop_broken_mobile_buttons=REMOVED'
...
if($ok){
 Write-Host 'V1018.16 LOCAL QA PASS' -ForegroundColor Green
 Write-Host '=== DEPLOYING OFFICIAL PRODUCTION ==='
 vercel.cmd --prod --yes --scope dayeem-studio-demos
 if($LASTEXITCODE -ne 0){Write-Host 'V1018.16 LOCAL PASS BUT PRODUCTION DEPLOY FAILED' -ForegroundColor Yellow;exit $LASTEXITCODE}
 $q=[string](Get-Date).Ticks
 $liveApp=(curl.exe -L -sS ('https://muscle-state-00.vercel.app/app.html?v=101816&t='+$q)) -join "`n"
 $liveCss=(curl.exe -L -sS ('https://muscle-state-00.vercel.app/assets/css/ms-v101815-clean-mobile.css?v=101816&t='+$q)) -join "`n"
 $liveRuntime=(curl.exe -L -sS ('https://muscle-state-00.vercel.app/assets/js/ms-v101815-clean-mobile.js?v=101816&t='+$q)) -join "`n"
 $liveData=(curl.exe -L -sS ('https://muscle-state-00.vercel.app/assets/js/data.js?v=101816&t='+$q)) -join "`n"
 $liveSw=(curl.exe -L -sS ('https://muscle-state-00.vercel.app/sw.js?v=101816&t='+$q)) -join "`n"
 if([string]::IsNullOrWhiteSpace($liveApp) -or [string]::IsNullOrWhiteSpace($liveData)){throw 'PRODUCTION_FETCH_FAILED'}
 if(!$liveApp.Contains('assets/js/data.js?v=101816') -or !$liveApp.Contains('ms-v101815-clean-mobile.css?v=101816') -or !$liveApp.Contains('ms-v101815-clean-mobile.js?v=101816')){throw 'PRODUCTION_APP_REFS_FAILED'}
 if(!$liveCss.Contains('.mobile-nav.ms938-nav{display:none!important}') -or !$liveCss.Contains('.ms101815-bottom-nav{display:none!important}')){throw 'PRODUCTION_DESKTOP_DOCK_HIDE_FAILED'}
 if(!$liveRuntime.Contains('if(!MOBILE.matches){n?.remove();return}')){throw 'PRODUCTION_RUNTIME_DOCK_GUARD_FAILED'}
 if(!$liveSw.Contains('muscle-state-pwa-v101816')){throw 'PRODUCTION_SW_101816_FAILED'}
 $liveRoot=Join-Path $env:TEMP ('MS_V101816_LIVE_'+$stamp);$liveDataDir=Join-Path $liveRoot 'assets\js';New-Item -ItemType Directory -Force -Path $liveDataDir | Out-Null;[IO.File]::WriteAllText((Join-Path $liveDataDir 'data.js'),$liveData,[Text.Encoding]::UTF8)
 node $qa $liveRoot;if($LASTEXITCODE -ne 0){throw 'PRODUCTION_TRAINING_QA_FAILED'}
 Write-Host 'PRODUCTION_ASSERTIONS=PASS'
 Write-Host 'V1018.16 PRODUCTION DEPLOY COMPLETED' -ForegroundColor Green
}