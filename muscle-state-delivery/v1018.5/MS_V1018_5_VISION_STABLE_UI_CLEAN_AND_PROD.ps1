...
$base='https://raw.githubusercontent.com/AhmedAbduldayeem/ArabfieldAcademy/'+$branch+'/muscle-state-delivery/v1018.5'
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$rb=Join-Path $root ('_rollback\v1018.5-vision-ui-'+$stamp)
...
Write-Host '=== MUSCLE STATE v1018.5 VISION STABLE + UI CLEAN ==='
...
$patch=Join-Path $env:TEMP 'MS_V10185_UI_CLEAN_PATCH.js'
$ok=$false
try{
 Invoke-WebRequest -UseBasicParsing -Uri ($base+'/ms-v10185-ui-clean-patch.js') -OutFile $patch -TimeoutSec 60
 $ph=Get-GitBlobSha1 $patch;Write-Host ('PATCH_GIT_BLOB_SHA1='+$ph);if($ph -ne 'da98f15b08c7af71f14d8f2ccb89c674218d66b0'){throw ('PATCH_BLOB_MISMATCH '+$ph)}
 ...
 if($f -match 'Gemini|OpenAI|Billing'){throw 'EXTERNAL_PROVIDER_COPY_IN_FOOD_UI'}
 if($r -match 'Gemini|OpenAI|Billing'){throw 'EXTERNAL_PROVIDER_COPY_IN_RESULT_UI'}
 if(!$h.Contains('ms-food-photo-v1018-1.js?v=10185')){throw 'FOOD_CACHE_10185_MISSING'}
 if(!$h.Contains('ms-food-photo-result-v10182.js?v=10185')){throw 'RESULT_CACHE_10185_MISSING'}
 if(!$h.Contains('assets/js/pwa.js?v=10185')){throw 'PWA_QUERY_10185_MISSING'}
 if(!$p.Contains("const VERSION='10185';")){throw 'PWA_VERSION_10185_MISSING'}
 Write-Host 'V1018_5_SOURCE_ASSERTIONS_OK'
 Write-Host 'user_ui=NO_EXTERNAL_PROVIDER_DATA'
 Write-Host 'analysis_copy=GENERIC_ONLY'
 Write-Host 'pwa_cache=10185'
 ...
 Write-Host ('V1018.5 ERROR: '+$_.Exception.Message) -ForegroundColor Red
...
if($ok){Write-Host 'V1018.5 LOCAL QA PASS' -ForegroundColor Green;Write-Host '=== DEPLOYING OFFICIAL PRODUCTION ===';vercel.cmd --prod --yes --scope dayeem-studio-demos;if($LASTEXITCODE -ne 0){Write-Host 'V1018.5 LOCAL PASS BUT PRODUCTION DEPLOY FAILED' -ForegroundColor Yellow;exit $LASTEXITCODE};Write-Host 'V1018.5 PRODUCTION DEPLOY COMPLETED' -ForegroundColor Green}